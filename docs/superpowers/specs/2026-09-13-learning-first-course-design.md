...
7. **代码说人话**——解释调用者、参数、状态变化和数据流，而不是只翻译语法。
8. **亲手代码实验**——学习者亲自修改或补写一小段核心 GDScript，先预测再运行。
9. **运行检查**——明确成功标准。
10. **如果失败**——3～5 个最高概率错误和排错入口。
11. **参数/行为实验**——再修改一个值或行为，强化直觉。
12. **学习检查**——3～5 个能够用自己的话回答的问题，至少一个要求复述调用链。
13. **可选挑战**——永远不作为下一课的前置条件。
14. **Checkpoint**——对应 Git Tag 和它代表的状态。
